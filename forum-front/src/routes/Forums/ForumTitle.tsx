...
function ForumTitle({ forumid, forumtitle, isLoggedIn, isRankForum }: ForumTitleProps) {
  const navigate = useNavigate();

  return (
    <>
      <div className="wrap" style={titleStyle} onClick={() => forumid && navigate(`/forum/${forumid}`)}>
        <div style={titleMainStyle}>
          ...
          {isLoggedIn && !isRankForum && (
            <Button
              onClick={(e) => {
                e.stopPropagation();
                if (forumid) navigate(`/forum/${forumid}/write`);
              }}
            >
              글 작성
            </Button>
