  ...
  useEffect(() => {
    ...
    Promise.allSettled([
      ...
      .then(([forumRes, postRes, commentsRes]) => {
        ...
        const postData = postRes.value.data;
              ...
              {post?.images?.length > 0 && (
                ...
                >
                  {post.images.map((img, idx) => (
                    <Image
                      key={img?.key || img?.img_key || img?.url || img?.img_URL || idx}
                      src={img.url || img.img_URL}
...
function PostHeader({ post, forumid, postid, setDeleteloading }) {
  ...
  const isOwn = String(authUserId) === String(post?.userId ?? post?.wby);

  const wby = post.user.username ?? `user#${post?.userId ?? 'unknown'}`;
  const wbyimg = post?.written?.profileImageUrl || post?.written?.profileImg?.img_URL || accountImg;
  const wtime = post?.createdAt ? timeAgo(post.createdAt) : (post?.wtime ? timeAgo(post.wtime) : "");
