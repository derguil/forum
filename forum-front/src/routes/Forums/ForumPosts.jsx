...
function ForumPosts() {
  ...
  useEffect(() => {
    ...
    if (tab) {
      ...
      axios.get("/api/posts", {
        params: { forumId: Number(forumid), page: currPage, limit }
      })
      .then(res => {
        setPosts(res.data);
        ...
        <div className={styles.postBody}>
          <div className={styles.postLeft}>
            ...
            <div className={`${styles.postMeta} ${isFirst ? styles.noDivider : ""}`}>
              ...
              <time className={`${styles.postTime} ${isFirst ? styles.noDivider : ""}`}>{timeAgo(post.createdAt)}</time>
              <h3 className={styles.postUser}>{post.user.username}</h3>
            </div>
          </div>
